(function(scope) {
    ...
    if (typeof pubsub === 'object') pubsubConfig = pubsub;
    else if (typeof global === 'object' && typeof global.pubsubConfig === 'object') pubsubConfig = global.pubsubConfig;
    function Pubsub(config) {
        ...
        function forEach(dataArray, callback) {
            var i = 0, arrayLength = dataArray.length;
            for(i = 0; i < arrayLength; i++)callback(i, dataArray[i]);
        }
        ...
        function executeCallback(subscriptions, args, async) {
            ...
            forEach(executedSubscriptions, function(subscriptionId, subscription) {
                if (typeof subscription === 'object' && executedSubscriptions.hasOwnProperty(subscriptionId)) {
                    ...
                    else subscription.callback.apply(subscription.context, args);
                }
            });
        }
        function executePublishWildcard(nsObject, args) {
            var nsElement;
            for(nsElement in nsObject)if (nsElement[0] !== '_' && nsObject.hasOwnProperty(nsElement)) executeCallback(nsObject[nsElement]._events, args);
        }
        function publish(nsObject, args, parts, params) {
            ...
            if (typeof nsObject['*'] !== 'undefined') publish(nsObject['*'], args, parts, params);
            // handle publish wildcard
            if (iPart === '*') executePublishWildcard(nsObject, args, async);
            // no namespace = leave publish
            if (typeof nsObject[iPart] === "undefined") {
                if (params.log) console.warn('There is no ' + params.nsString + ' subscription');
                return;
            }
            ...
            } else if (recurrent === true && typeof depth === 'number' && partNumber >= partsLength - depth) executeCallback(nsObject._events, args, async);
        ...
        function executeSubscribeWildcard(nsObject, args, params) {
            ...
            var async = params.async;
            var nextPart = null;
            if (parts.length === 0) executeCallback(nsObject._events, args, async);
            else {
                nextPart = parts.shift();
                if (nsObject[nextPart]) executeSubscribeWildcard(nsObject[nextPart], args, {
                    parts: parts,
                    async: async,
        ...
        function subscribe(nsString, callback, params) {
            var parts = nsString.split(options.separator), nsObject, context = params && typeof params.context !== 'undefined' ? params.context : options.context, eventObject = null, i = 0;
        ...
        function unsubscribe(subscribeObject) {
            if (subscribeObject === null || typeof subscribeObject === 'undefined') return null;
            var nsString = subscribeObject.namespace, eventObject = subscribeObject.event, parts = nsString.split(options.separator), nsObject, i = 0;
            //Iterating through _eventObject to find proper nsObject
            nsObject = _eventObject;
            for(i = 0; i < parts.length; i += 1){
                if (typeof nsObject[parts[i]] === "undefined") {
                    if (options.log) console.error('There is no ' + nsString + ' subscription');
                    return null;
                }
                nsObject = nsObject[parts[i]];
            }
            forEach(nsObject._events, function(eventId) {
                if (nsObject._events[eventId] === eventObject) nsObject._events.splice(eventId, 1);
            });
        }
        return {
            /**
			 ...
			 */ publish: function(nsString, args, params) {
                var parts = nsString.split(options.separator), recurrent = typeof params === 'object' && params.recurrent ? params.recurrent : options.recurrent, depth = typeof params === 'object' && params.depth ? params.depth : options.depth, async = typeof params === 'object' && params.async ? params.async : options.async, partsLength = parts.length;
                if (!parts.length) {
                    if (options.log) console.error('Wrong namespace provided ' + nsString);
                    return;
                }
			 ...
			 */ subscribeOnce: function(nsString, callback, params) {
                var self = this, subscription = null;
			 ...
			 */ unsubscribe: function(subscribeObject) {
                ...
                if (isArray(subscribeObject)) forEach(subscribeObject, function(number) {
                    var oneSubscribtion = subscribeObject[number];
                    unsubscribe.apply(self, [
                        oneSubscribtion
                    ]);
                });
                else unsubscribe.apply(self, arguments);
            },
            /**
    ...
    if (typeof define === 'function') define(pubsubInstance);
    //node.js
    if (typeof module === 'object' && module.exports) module.exports = pubsubInstance;
    if (typeof window === 'object') {
        window.pubsub = pubsubInstance;
        if (window !== scope) scope.pubsub = pubsubInstance;
    }
})(this);

//# sourceMappingURL=index.bbfee37b.js.map
